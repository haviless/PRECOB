...
procedure TFproinfpla.FormActivate(Sender: TObject);
...
procedure TFproinfpla.dbgcabeceraCellChanged(Sender: TObject);
begin
  actdetpla;
end;

procedure TFproinfpla.dbgcabeceraIButtonClick(Sender: TObject);
begin
  DMPreCob.xCnd := 'I';
  pnlCab.Visible := True;
  pnlCab.Left := 54;
...
procedure TFproinfpla.bbtnOkCClick(Sender: TObject);
...
procedure TFproinfpla.bbtnCancCClick(Sender: TObject);
begin
  pnlCab.Visible := False;
end;

procedure TFproinfpla.actcabpla;
...
begin
  ...
  Begin
    ...
    xflg := 'S';
  End;
  xSql := 'SELECT PLANTILLA, NOMBRE FROM COB_INF_PLA_MAS_CAB A WHERE OFDESID = '+QuotedStr(DMPreCob.wOfiId)
  +' AND TIPO = '+QuotedStr(DMPreCob.cdsCredito.FieldByName('TIPO').AsString);
...
procedure TFproinfpla.dbgcabeceraDblClick(Sender: TObject);
begin
  DMPreCob.xCnd := 'M';
  pnlCab.Visible := True;
...
procedure TFproinfpla.actdetpla;
...
begin
  ...
  xSql := 'SELECT PLANTILLA, ITEM, CAMPO, NOMBRE, DESDE, HASTA, TIPCAM FROM COB_INF_PLA_MAS_DET WHERE PLANTILLA = '+QuotedStr(DMPreCob.cdsQry5.FieldByName('PLANTILLA').AsString)
  +' ORDER BY ITEM';
...
procedure TFproinfpla.dbgdetalleIButtonClick(Sender: TObject);
begin
  DMPreCob.xCnd := 'I';
  dblcdnomcam.Text := '';
  medescam.Text    := '';
...
procedure TFproinfpla.bbtnCancDClick(Sender: TObject);
begin
  pnlDet.Visible := False;
end;

procedure TFproinfpla.dblcdnomcamExit(Sender: TObject);
begin
  medescam.Text := DMPreCob.cdsQry3.FieldByName('DESCAM').AsString;
end;

procedure TFproinfpla.FormKeyPress(Sender: TObject; var Key: Char);
begin
  If key=#13 Then
  Begin
    key:=#0;
...
procedure TFproinfpla.bbtnOkDClick(Sender: TObject);
Var xSql:String;
...
procedure TFproinfpla.dbgdetalleDblClick(Sender: TObject);
begin
    DMPreCob.xCnd := 'M';
    pnlDet.Visible   := True;
...
procedure TFproinfpla.btntransferirClick(Sender: TObject);
Var xusenom, xSql, xflg:String;
    xpos:Integer;
    xcampos:String;
    //xmonenv:Double;
begin
...
// Verificando si existe el campo ASOCODPAGO
  xSql := 'SELECT * FROM COB_INF_PLA_MAS_DET WHERE PLANTILLA = '+QuotedStr(DMPreCob.cdsQry5.FieldByName('PLANTILLA').AsString)
...
// Verificando si existe el campo USEID
  xSql := 'SELECT * FROM COB_INF_PLA_MAS_DET WHERE PLANTILLA = '+QuotedStr(DMPreCob.cdsQry5.FieldByName('PLANTILLA').AsString)
  +' AND CAMPO = ''USEID''';
  DMPreCob.cdsQry.Close;
  DMPreCob.cdsQry.DataRequest(xSql);
  ...
  If DMPreCob.cdsQry.RecordCount <> 1 Then
  Begin
    MessageDlg('La plantilla seleccionada no contiene el campo USEID'#13'Incluya el campo dentro de la plantilla', mtError, [mbOk], 0);
    Exit;
  End;
  Screen.Cursor := crHourGlass;
  ...
  Begin
    ...
    If DMPreCob.cdsQry6.FieldByName('TIPCAM').AsString = 'C' Then
    Begin
    ...
    Begin
      xSql := xSql + DMPreCob.cdsQry6.FieldByName('CAMPO').AsString
      +' = TO_NUMBER(SUBSTR(LINEA,'+DMPreCob.cdsQry6.FieldByName('DESDE').AsString+','+IntToStr(xpos)+'),''9999.99'')';
      xflg := 'X';
    End;
...
procedure TFproinfpla.BitBtn3Click(Sender: TObject);
begin
  Fproinfpla.Close;
end;
...
procedure TFproinfpla.btneliregClick(Sender: TObject);
Var xSql:String;
begin
  // Elimina registro
